...
def _parse_title(title: str) -> dict:
    ...
    for tok in sorted(_MODEL_TOKENS, key=len, reverse=True):
        if re.search(rf"\b{re.escape(tok)}\b", clean, re.I):
            ...
            # Strip exclamation-mark-heavy noise (e.g. "1 Owner! PPF! Huge" → stop at first !)
            if trim.count('!') >= 2:
                trim = trim.split('!')[0].strip()
            elif '!' in trim:
                trim = re.sub(r'\s*!.*$', '', trim).strip()
            # Strip trailing 'for sale', 'available', 'priced to sell' noise
            trim = re.sub(r'\s+(?:for sale|available|priced to sell|contact|call|dm|pm).*$', '', trim, flags=re.I).strip()
            # Strip trailing dash-separated noise (e.g. "4 - Couple" → "4")  
            trim = re.sub(r'\s*-\s+[A-Za-z].*$', '', trim).strip()
