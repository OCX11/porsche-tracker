...
from core.db import get_conn, get_dashboard_data, init_db, source_category
    ...
    with get_conn() as conn:
        ...
        for c in comps:
            comp_items.append({
                ...
                "model": c.get("model", "") or "",
                "trim":  c.get("trim", "") or "",
                "gen":   c.get("generation") or _gen(c.get("year"), c.get("model")) or "",
                "trans": c.get("transmission", "") or "",
                "mi":    c.get("mileage"),
                "pr":    c.get("sold_price") or c.get("sale_price"),
                "dt":    (c.get("sold_date") or c.get("sale_date") or "")[:10],
                "url":   c.get("listing_url", "") or "#",
            })
        comp_data_json = _json.dumps(comp_items, ensure_ascii=False)
...
.card-expand {{ display:none; padding:10px 12px 12px; border-top:1px solid #1E1E2A; background:#0D0D11; }}
.card.expanded .card-expand {{ display:block; }}
.expand-graph-label {{ font-family:'DM Mono',monospace; font-size:10px; color:#555; letter-spacing:.5px; text-transform:uppercase; margin-bottom:6px; display:flex; justify-content:space-between; }}
.expand-cc {{ color:#444; letter-spacing:0; text-transform:none; }}
.expand-graph-svg {{ width:100%; height:130px; display:block; }}
.expand-goto {{ display:inline-block; margin-top:8px; font-family:'DM Mono',monospace; font-size:11px; color:#60A5FA; text-decoration:none; }}
.expand-goto:hover {{ color:#93C5FD; }}
.card-top-row {{
...
<div class="dropdown-overlay" id="dd-overlay">
  ...
  <div class="dropdown">
    <a class="dd-item" href="saved.html"><span class="dd-icon">&#x2605;</span> My Cars</a>
...
<script>
// ── Card data — loaded from listings.json at DOMContentLoaded ─────────────────
var CARD_DATA = [];
var visibleCards = [];
var renderedCount = 0;
var PAGE = 48;

// ── Comp data — loaded from comps.json, rendered lazily on first Comps view ──
var COMP_DATA = [];
var _compsRendered = false;
var _compsViewRequested = false;
...
function _renderComps() {{
  ...
  if (!body || !COMP_DATA.length) return;
  var rows = '';
  for (var i = 0; i < COMP_DATA.length; i++) {{
    var c = COMP_DATA[i];
    var sk = (c.src || '').toLowerCase();
    var bc = _COMP_BADGE[sk] || ['#18181F','#6B6B7D',_escHtml((c.src||'?').slice(0,12))];
    var badge = '<span class="badge" style="background:'+bc[0]+';color:'+bc[1]+'">'+bc[2]+'</span>';
    var mi = c.mi ? Number(c.mi).toLocaleString()+' mi' : '—';
    var pr = c.pr ? '$'+Number(c.pr).toLocaleString() : '—';
    rows += '<tr class="comp-row" data-gen="'+_escHtml(c.gen)+'" data-year="'+c.yr+'" data-model="'+_escHtml(c.model)+'">';
    rows += '<td>'+badge+'</td>';
    rows += '<td>'+_escHtml(c.yr)+'</td>';
    rows += '<td class="td-model">'+_escHtml(c.model)+' '+_escHtml(c.trim)+'</td>';
    rows += '<td>'+_escHtml(c.gen)+'</td>';
    rows += '<td>'+_escHtml(c.trans||'—')+'</td>';
    rows += '<td>'+mi+'</td>';
    rows += '<td class="td-price">'+pr+'</td>';
    rows += '<td>'+(c.dt||'—')+'</td>';
    rows += '<td><a href="'+_escHtml(c.url)+'" target="_blank" rel="noopener" onclick="event.stopPropagation()" class="tbl-link">&#x2192;</a></td>';
    rows += '</tr>';
  }}
  body.innerHTML = rows;
  _compsRendered = true;
...
function switchView(name, btn) {{
  ...
  if (name === 'comps') {{
    if (!_compsRendered) {{
      if (COMP_DATA.length) {{ _renderComps(); }}
      else {{ _compsViewRequested = true; }}
    }}
  }}
}}
...
// ── Smart auto-refresh — re-fetch listings.json (not HTML scrape) ────────────
function _autoRefresh() {{
  fetch('listings.json?_nc=' + Date.now(), {{cache:'no-store'}})
    .then(function(r) {{ return r.json(); }})
    .then(function(fresh) {{
      if (!Array.isArray(fresh) || fresh.length === 0) return;
      // Update if count changed OR if newest listing ID changed
      var oldTop = CARD_DATA.length > 0 ? CARD_DATA[0].id : null;
      var newTop = fresh.length > 0 ? fresh[0].id : null;
      if (fresh.length === CARD_DATA.length && oldTop === newTop) return;
      CARD_DATA = fresh;
      applyFilters();
    }}).catch(function() {{}});
}}
setInterval(_autoRefresh, 120000);
...
function toggleStar(btn) {{
  ...
  if (typeof toggleSave === 'function' && typeof _user !== 'undefined' && _user) {{
    ...
    var price = card ? parseInt(card.dataset.price||'0')||null : null;
    var model = card ? (card.dataset.year+' Porsche '+card.dataset.model) : '';
    toggleSave(lid, {{ listing_url: url, url: url, price: price, title: model }});
...
window.addEventListener('DOMContentLoaded', function() {{
  ...
  fetch('listings.json')
    ...
    .catch(function(e) {{ console.error('listings.json load failed', e); }});

  fetch('comps.json')
    .then(function(r) {{ return r.json(); }})
    .then(function(data) {{
      COMP_DATA = data;
      if (_compsViewRequested) _renderComps();
    }})
    .catch(function(e) {{ console.error('comps.json load failed', e); }});
}});
</script>
</html>"""
    ...
    # Write external data files (fetched by dashboard at runtime)
    (BASE_DIR / "docs" / "listings.json").write_text(card_data_json, encoding="utf-8")
    (BASE_DIR / "docs" / "comps.json").write_text(comp_data_json, encoding="utf-8")
