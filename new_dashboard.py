...
window.addEventListener('pageshow', function(e) {{
  if (e.persisted) {{
    // iOS WKWebView can blank its render layer on bfcache restore.
    // Force a reflow to repaint, then re-render cards if data is loaded.
    document.body.style.display = 'none';
    void document.body.offsetHeight;
    document.body.style.display = '';
    if (CARD_DATA.length > 0 && typeof applyFilters === 'function') applyFilters();
  }}
}});

// PWA visibility restore — when returning from an external app (eBay, BaT, etc.)
// iOS fires visibilitychange. Re-fetch listings.json (hits SW cache instantly)
// and re-render, which cures the blank WKWebView on iOS 16.4+.
document.addEventListener('visibilitychange', function() {{
  if (document.visibilityState === 'visible') {{
    fetch('listings.json?_=' + Date.now())
      .then(function(r) {{ return r.json(); }})
      .then(function(data) {{ CARD_DATA = data; if (typeof applyFilters === 'function') applyFilters(); }})
      .catch(function() {{ if (CARD_DATA.length > 0 && typeof applyFilters === 'function') applyFilters(); }});
  }}
}});
