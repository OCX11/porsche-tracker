...
def cleanup_stale_retail_listings(conn, days=14):
    """Mark retail listings not seen in  days as sold.
    Skips auction sources (BaT, C&B, pcarmarket) — those are protected by the
    auction guard and have their own comp-promotion flow.
    Returns count of listings cleaned up."""
    _AUCTION_DEALERS = ("Bring a Trailer", "Cars and Bids", "pcarmarket")
    placeholders = ",".join("?" * len(_AUCTION_DEALERS))
    cutoff = (date.today() - timedelta(days=days)).isoformat()
    result = conn.execute(
        f"""UPDATE listings
           SET status='sold', archived_at=datetime('now'), archive_reason='stale_{days}d'
           WHERE status='active'
             AND date_last_seen < ?
             AND date_last_seen IS NOT NULL
             AND dealer NOT IN ({placeholders})""",
        (cutoff, *_AUCTION_DEALERS)
    )
    conn.commit()
    return result.rowcount
